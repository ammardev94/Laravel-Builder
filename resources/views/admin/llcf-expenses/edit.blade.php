    ...
    <div class="container-fluid">
        ...
        <div class="card card-primary">
            <div class="card-header">
                <h3 class="card-title">Edit LLCF Expense</h3>
            </div>

            <form action="{{ route('admin.expenses-llcf.update', $expense->id) }}" method="POST" id="editExpenseForm">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select id="item_id" name="item_id" class="form-control" required>
                                ...
                                @foreach($items as $item)
                                    <option value="{{ $item->id }}" {{ old('item_id', $expense->item_id) == $item->id ? 'selected' : '' }}>
                                        {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" step="0.01" name="amount" id="amount" class="form-control"
                                   placeholder="Enter expense amount" value="{{ old('amount', $expense->amount) }}" required>
                            @error('amount')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
